...
contract ZLottery is SepoliaConfig {
    ...
    // Winning number for each round (encrypted)
    mapping(uint256 => euint8) public winningNumbers;
    ...
    function drawLottery() external onlyOwner {
        ...
        // Use unbounded random and map to range using modulo
        euint8 randomValue = FHE.randEuint8();
        
        // Calculate range (11-99 = 89 values)
        uint8 range = MAX_NUMBER - MIN_NUMBER + 1; // 89
        
        // Map to range using scalar rem operation
        euint8 mappedValue = FHE.rem(randomValue, range);
        euint8 winningNumber = FHE.add(mappedValue, FHE.asEuint8(MIN_NUMBER));
        
        winningNumbers[currentRound] = winningNumber;
        isRoundDrawn[currentRound] = true;
        
        // Grant permissions for the winning number
    ...
    function checkTicket(uint256 round, uint256 ticketIndex) external returns (ebool) {
        ...
        euint8 winning = winningNumbers[round];
        
        ebool isWinner = FHE.eq(userTicket, winning);
    ...
    function processClaimDecryption(
        uint256 requestId,
        bool isWinnerDecrypted,
    ...
    function claimPrizeSimple(uint256 round, uint256 ticketIndex) external {
        ...
        // User must have access to both their ticket and winning number to compare
        euint8 userTicket = userTickets[round][msg.sender][ticketIndex];
        euint8 winning = winningNumbers[round];
        
        require(FHE.isSenderAllowed(userTicket), "No access to ticket");
        require(FHE.isSenderAllowed(winning), "No access to winning number");
    ...
    /// @notice Make winning number publicly decryptable for a specific round
    /// @param round The lottery round to make public
    /// @dev Only owner can make winning numbers public
    function makeWinningNumberPublic(uint256 round) external onlyOwner {
        require(isRoundDrawn[round], "Round not drawn yet");
        euint8 winning = winningNumbers[round];
        FHE.makePubliclyDecryptable(winning);
    }
